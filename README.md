...
- **`api/`** has zero third-party dependencies. It doesn't build SQL from
  ...
  protobuf tile, clipped with a 64-unit buffer (of the 4096 extent) so
  polygons that cross a tile edge are cut well outside the visible tile
  instead of right on the seam, where the outline layer would otherwise
  draw the cut as a line. The same buffer is passed to `MVTBoundingBox` as
  its margin so the bbox prefilter also picks up parcels sitting just past
  the edge. Each feature's properties include `av_per_sqft`
