...
// area_sqft is included in every tile feature's properties so the map can
// compute assessed-value-per-sqft client-side (cheap MapLibre paint-time
...
SELECT MVTEncode('parcels')(
    ...
    tuple(id, landuse, zonedist_simple, assessland, assesstot, numfloors, yearbuilt,
          unitsres, bldgclass_simple, yearalter1, builtfar, area_sqft)
...
// per square foot (assesstot / area_sqft — area_sqft is precomputed once at
// load time, see elt/load_pluto.py, not recomputed from geometry here)
...
// huge parcels from dominating the color scale.
...
SELECT
    ...
    arrayMap(x -> round(x, 1), quantiles(0.2, 0.4, 0.6, 0.8)(assesstot / nullif(area_sqft, 0))) AS value_breaks
FROM pluto_parcels
...
SELECT
    ...
    yearbuilt, unitsres, assessland, assesstot, builtfar, area_sqft,
    round(assesstot / nullif(area_sqft, 0), 2) AS av_per_sqft
FROM pluto_parcels
WHERE min_lon <= {maxLon:Float64} AND max_lon >= {minLon:Float64}
  AND min_lat <= {maxLat:Float64} AND max_lat >= {minLat:Float64}
